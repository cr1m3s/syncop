...
def getServers():
    ...
    result = subprocess.run(
        ["dhcp-lease-list | grep ILO | awk '{print $2,$3}'"],
...
def write_hosts(servers):
    ...
    with open(ANSIBLE_HOSTS, "a", encoding="utf-8") as file:
        for key in servers:
            print(
                f'ILO{key}\tbaseuri={servers[key]["IP"]}\tpassword={servers[key]["PSWD"]}\n'
            )
            file.write(
                f'ILO{key}\tbaseuri={servers[key]["IP"]}\tpassword={servers[key]["PSWD"]}\n'
            )
...
def power_state(servers):
    ...
    with ThreadPoolExecutor(max_workers=len(servers)) as executor:
        future_to_server = {
            executor.submit(
                check_power_state, server_name, server_info, username
            ): server_name
    ...
    for result in results:
        ...
        if returncode != 0:
            ...
            print(
                f'{server_name}:\t{servers[server_name]["IP"]}\t{username}\t{servers[server_name]["PSWD"]}'
            )
        else:
...
def ansible_logs():
    ...
    global LO
    log_path = f"/data/log/NokiaNSW/Ansible/ansible_{LO}.log"
...
def gen10_menu():
    ...
    if choice == "1":
        ...
        os.system(
            "cp /var/www/html/boot/script_menu_gen10_SPP2023.09.03.pl /var/www/html/boot/script_menu.pl"
        )
    elif choice == "4":
...
def gen11_menu():
    ...
    if choice == "1":
        ...
        os.system(
            "cp /var/www/html/boot/script_menu_gen11_SPP2023.10.00.pl /var/www/html/boot/script_menu.pl"
        )
    elif choice == "2":
        MODEL = "P73549-B21    CBIS 24         SPP 2024.04.00.00 Gen11"
        os.system(
            "cp /var/www/html/boot/script_menu_gen11_SPP2024.04.00.pl /var/www/html/boot/script_menu.pl"
        )
    elif choice == "3":
        MODEL = "P73550-B21    NCP 23.11       SPP 2023.10.00.02 Gen11"
        os.system(
            "cp /var/www/html/boot/script_menu_gen11_SPP2023.10.02.pl /var/www/html/boot/script_menu.pl"
        )
    elif choice == "0":
        main_menu()
    else:
