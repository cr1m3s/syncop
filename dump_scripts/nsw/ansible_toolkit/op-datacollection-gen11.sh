...
# Checking drives at Storage Nodes - scan JSON files with MR216 controller, check for 6 NVMe drives on Intel VROC and 2 SATA SSDs on MR216
echo "# Checking drives at Storage Nodes #"
error=0
...
#for f in $(grep "MR216" /data/log/NokiaNSW/*.json | awk -F ":" '{print $1}' | uniq)
for f in $(grep -ril MR216 /data/log/NokiaNSW/*.json)
  do
	# disk_sata=$(cat $f | grep -c "\"StructuredBootString\"\: \"HD.EmbSATA")
	# disk_sas=$(cat $f | grep -c "\"StructuredBootString\"\: \"HD.EmbRAID")
	disk_nvme=$(cat $f | grep -c "\"StructuredBootString\"\: \"NVMe")
	disk_slots=$(cat $f | grep -c "\"StructuredBootString\"\: \"HD.Slot")

	if [[ ${disk_nvme} != 6 ]] || [[ ${disk_slots} != 2 ]]; then
      echo "- Unit $(echo $f | awk -F "_" '{print substr($3,1,10)}') FAILED" |tee -a /data/sfng/log/${HPON}/disk_check.txt
  	  echo "   VROC  - NVMe: $disk_nvme" |tee -a /data/sfng/log/${HPON}/disk_check.txt
	  echo "   MR216 - SATA: $disk_slots" |tee -a /data/sfng/log/${HPON}/disk_check.txt
	  error=1
	else
      echo "- Unit $(echo $f | awk -F "_" '{print substr($3,1,10)}') is OK" |tee -a /data/sfng/log/${HPON}/disk_check.txt
  	  echo "   VROC  - NVMe: $disk_nvme" |tee -a /data/sfng/log/${HPON}/disk_check.txt
	  echo "   MR216 - SATA: $disk_slots" |tee -a /data/sfng/log/${HPON}/disk_check.txt
	fi
  done
...
if [[ ${error} != 0 ]]; then 
	echo -e "\nPlease contact HPE support to check disk connections.\n" |tee -a /data/sfng/log/${HPON}/disk_check.txt
	#exit 1
	fi

echo -e "\n"
read -p "Disk check done, press any key to continue ..." -n1 -s
echo -e "\n"
...
ilorest_params=("10/25Gb 2P 641SFP28 Adptr" "MCX5" "iLO 5 :" "iLO 6 :" "Intel(R) Eth" "System ROM" "MR216" "ConnectX-6")
# OLD data:
# ilorest_params=("Intelligent Platform Abstraction Data" "Intelligent Provisioning" "Mellanox ConnectX-6 LX OCP3.0" "10/25Gb 2P 641SFP28 Adptr" "MCX5" "iLO 5 :" "Innovation Engine (IE) Firmware" "Server Platform Services (SPS) Firmware" "Server Platform Services (SPS) Descriptor" "System ROM")
